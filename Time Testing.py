import time
import alpaca_trade_api as tradeapi
from datetime import datetime



private_key = '0L3k7yqOfK0ncoMwjrsV5PaDKru00lUkyFgwwHCK'
public_key = 'PKO1OGDZUBL0QS9N1AT2'
base_url = 'https://paper-api.alpaca.markets'
currentTicker = ''

all_stocks = ['AACG',
'AACI',
'AACIU',
'AACIW',
'AADI',
'AAL',
'AAME',
'AAOI',
'AAON',
'AAPL',
'AATC',
'AAWW',
'ABCB',
'ABCL',
'ABCM',
'ABEO',
'ABGI',
'ABIO',
'ABMD',
'ABNB',
'ABOS',
'ABSI',
'ABST',
'ABTX',
'ABUS',
'ABVC',
'ACAB',
'ACABU',
'ACABW',
'ACAD',
'ACAH',
'ACAHU',
'ACAHW',
'ACAX',
'ACAXR',
'ACAXW',
'ACB',
'ACBAU',
'ACBAW',
'ACCD',
'ACER',
'ACET',
'ACEV',
'ACEVU',
'ACEVW',
'ACGL',
'ACGLN',
'ACGLO',
'ACHC',
'ACHL',
'ACHV',
'ACIU',
'ACIW',
'ACKIT',
'ACKIU',
'ACKIW',
'ACLS',
'ACLX',
'ACMR',
'ACNB',
'ACON',
'ACONW',
'ACOR',
'ACQR',
'ACQRU',
'ACQRW',
'ACRS',
'ACRX',
'ACST',
'ACT',
'ACTD',
'ACTDU',
'ACTDW',
'ACTG',
'ACVA',
'ACXP',
'ADAG',
'ADAL',
'ADALU',
'ADALW',
'ADAP',
'ADBE',
'ADER',
'ADERU',
'ADERW',
'ADES',
'ADGI',
'ADI',
'ADIL',
'ADILW',
'ADMA',
'ADMP',
'ADN',
'ADNWW',
'ADOC',
'ADOCR',
'ADOCW',
'ADP',
'ADPT',
'ADSE',
'ADSEW',
'ADSK',
'ADTH',
'ADTHW',
'ADTN',
'ADTX',
'ADUS',
'ADV',
'ADVM',
'ADVWW',
'ADXN',
'AEAC',
'AEACU',
'AEACW',
'AEAE',
'AEAEU',
'AEAEW',
'AEHA',
'AEHAW',
'AEHL',
'AEHR',
'AEI',
'AEIS',
'AEMD',
'AEP',
'AEPPZ',
'AERC',
'AERI',
'AESE',
'AEY',
'AEYE',
'AEZS',
'AFAC',
'AFACU',
'AFACW',
'AFAQ',
'AFAQW',
'AFARU',
'AFBI',
'AFCG',
'AFIB',
'AFMD',
'AFRM',
'AFYA',
'AGBA',
'AGBAR',
'AGEN',
'AGFS',
'AGFY',
'AGGR',
'AGGRW',
'AGIL',
'AGILW',
'AGIO',
'AGLE',
'AGMH',
'AGNC',
'AGNCM',
'AGNCN',
'AGNCO',
'AGNCP',
'AGRI',
'AGRIW',
'AGRX',
'AGTC',
'AGYS',
'AHCO',
'AHG',
'AHI',
'AHPA',
'AHPAU',
'AHPAW',
'AHPI',
'AHRN',
'AHRNU',
'AHRNW',
'AIB',
'AIBBR',
'AIBBU',
'AIH',
'AIHS',
'AIKI',
'AIMAU',
'AIMC',
'AINV',
'AIP',
'AIRG',
'AIRS',
'AIRT',
'AIRTP',
'AKAM',
'AKAN',
'AKBA',
'AKIC',
'AKICU',
'AKICW',
'AKRO',
'AKTS',
'AKTX',
'AKU',
'AKUS',
'AKYA',
'ALBO',
'ALCO',
'ALDX',
'ALEC',
'ALF',
'ALFIW',
'ALGM',
'ALGN',
'ALGS',
'ALGT',
'ALHC',
'ALIM',
'ALJJ',
'ALKS',
'ALKT',
'ALLK',
'ALLO',
'ALLR',
'ALLT',
'ALNA',
'ALNY',
'ALOR',
'ALORU',
'ALORW',
'ALOT',
'ALPA',
'ALPAU',
'ALPAW',
'ALPN',
'ALPP',
'ALR',
'ALRM',
'ALRN',
'ALRS',
'ALSA',
'ALSAR',
'ALSAU',
'ALSAW',
'ALT',
'ALTO',
'ALTR',
'ALTU',
'ALTUU',
'ALTUW',
'ALVR',
'ALXO',
'ALYA',
'ALZN',
'AMAL',
'AMAO',
'AMAOW',
'AMAT',
'AMBA',
'AMCI',
'AMCIW',
'AMCX',
'AMD',
'AMED',
'AMEH',
'AMGN',
'AMKR',
'AMLX',
'AMNB',
'AMOT',
'AMPG',
'AMPGW',
'AMPH',
'AMPL',
'AMRK',
'AMRN',
'AMRS',
'AMSC',
'AMSF',
'AMST',
'AMSWA',
'AMTB',
'AMTI',
'AMTX',
'AMWD',
'AMYT',
'AMZN',
'ANAB',
'ANAT',
'ANDE',
'ANEB',
'ANGH',
'ANGHW',
'ANGI',
'ANGN',
'ANGO',
'ANIK',
'ANIP',
'ANIX',
'ANNX',
'ANPC',
'ANSS',
'ANTE',
'ANTX',
'ANY',
'ANZU',
'ANZUU',
'ANZUW',
'AOGO',
'AOGOU',
'AOGOW',
'AOSL',
'AOUT',
'APA',
'APAC',
'APACU',
'APACW',
'APCX',
'APCXW',
'APDN',
'APEI',
'APEN',
'API',
'APLD',
'APLS',
'APLT',
'APM',
'APMI',
'APMIU',
'APMIW',
'APOG',
'APP',
'APPF',
'APPH',
'APPHW',
'APPN',
'APPS',
'APRE',
'APTM',
'APTMU',
'APTMW',
'APTO',
'APTX',
'APVO',
'APWC',
'APXI',
'APXIU',
'APXIW',
'APYX',
'AQB',
'AQMS',
'AQST',
'ARAV',
'ARAY',
'ARBE',
'ARBEW',
'ARBG',
'ARBGU',
'ARBGW',
'ARBK',
'ARBKL',
'ARCB',
'ARCC',
'ARCE',
'ARCK',
'ARCKU',
'ARCKW',
'ARCT',
'ARDS',
'ARDX',
'AREB',
'AREBW',
'AREC',
'ARGUU',
'ARGX',
'ARHS',
'ARIZ',
'ARIZR',
'ARIZU',
'ARKO',
'ARKOW',
'ARKR',
'ARLP',
'AROW',
'ARQQ',
'ARQQW',
'ARQT',
'ARRW',
'ARRWU',
'ARRY',
'ARTA',
'ARTAU',
'ARTAW',
'ARTE',
'ARTEU',
'ARTEW',
'ARTL',
'ARTLW',
'ARTNA',
'ARTW',
'ARVL',
'ARVN',
'ARWR',
'ARYD',
'ASAX',
'ASAXU',
'ASCA',
'ASCAR',
'ASCAU',
'ASLE',
'ASLN',
'ASMB',
'ASML',
'ASND',
'ASO',
'ASPA',
'ASPAW',
'ASPC',
'ASPCU',
'ASPCW',
'ASPS',
'ASPU',
'ASRT',
'ASRV',
'ASTC',
'ASTE',
'ASTL',
'ASTLW',
'ASTR',
'ASTS',
'ASTSW',
'ASUR',
'ASYS',
'ATAI',
'ATAK',
'ATAKR',
'ATAKU',
'ATAKW',
'ATAX',
'ATCOL',
'ATCX',
'ATEC',
'ATER',
'ATEX',
'ATHA',
'ATHE',
'ATHX',
'ATIF',
'ATLC',
'ATLCL',
'ATLCP',
'ATLO',
'ATNF',
'ATNFW',
'ATNI',
'ATNX',
'ATOM',
'ATOS',
'ATRA',
'ATRC',
'ATRI',
'ATRO',
'ATRS',
'ATSG',
'ATSPT',
'ATSPU',
'ATSPW',
'ATVC',
'ATVCU',
'ATVCW',
'ATVI',
'ATXI',
'ATXS',
'ATY',
'AUB',
'AUBAP',
'AUBN',
'AUDC',
'AUGX',
'AUID',
'AUPH',
'AUR',
'AURA',
'AURC',
'AURCU',
'AURCW',
'AUROW',
'AUTL',
'AUTO',
'AUUD',
'AUUDW',
'AUVI',
'AUVIP',
'AVAC',
'AVACU',
'AVACW',
'AVAH',
'AVAV',
'AVCO',
'AVCT',
'AVCTW',
'AVDL',
'AVDX',
'AVEO',
'AVGO',
'AVGOP',
'AVGR',
'AVHI',
'AVHIU',
'AVHIW',
'AVID',
'AVIR',
'AVNW',
'AVO',
'AVPT',
'AVPTW',
'AVRO',
'AVT',
'AVTE',
'AVTX',
'AVXL',
'AWH',
'AWRE',
'AXDX',
'AXGN',
'AXLA',
'AXNX',
'AXON',
'AXSM',
'AXTI',
'AY',
'AYLA',
'AYRO',
'AYTU',
'AZ',
'AZN',
'AZPN',
'AZTA',
'AZYO',
'BAFN',
'BAND',
'BANF',
'BANFP',
'BANR',
'BANX',
'BAOS',
'BASE',
'BATRA',
'BATRK',
'BBBY',
'BBCP',
'BBGI',
'BBI',
'BBIG',
'BBIO',
'BBLG',
'BBLGW',
'BBQ',
'BBSI',
'BCAB',
'BCAC',
'BCACU',
'BCACW',
'BCBP',
'BCDA',
'BCDAW',
'BCEL',
'BCLI',
'BCML',
'BCOR',
'BCOV',
'BCOW',
'BCPC',
'BCRX',
'BCSA',
'BCSAU',
'BCSAW',
'BCTX',
'BCTXW',
'BCYC',
'BDSX',
'BDTX',
'BEAM',
'BEAT',
'BEATW',
'BECN',
'BEEM',
'BEEMW',
'BELFA',
'BELFB',
'BENE',
'BENER',
'BENEU',
'BENEW',
'BFC',
'BFI',
'BFIIW',
'BFIN',
'BFRI',
'BFRIW',
'BFST',
'BGCP',
'BGFV',
'BGNE',
'BGRY',
'BGRYW',
'BHAC',
'BHACU',
'BHACW',
'BHAT',
'BHF',
'BHFAL',
'BHFAM',
'BHFAN',
'BHFAO',
'BHFAP',
'BHSE',
'BHSEU',
'BHSEW',
'BIDU',
'BIGC',
'BIIB',
'BILI',
'BIMI',
'BIOC',
'BIOL',
'BIOS',
'BIOSU',
'BIOSW',
'BIOT',
'BIOTU',
'BIOTW',
'BIOX',
'BIRD',
'BITF',
'BIVI',
'BJDX',
'BJRI',
'BKCC',
'BKEP',
'BKEPP',
'BKNG',
'BKR',
'BKSC',
'BKYI',
'BL',
'BLBD',
'BLBX',
'BLCM',
'BLCT',
'BLDE',
'BLDEW',
'BLDP',
'BLEU',
'BLEUR',
'BLEUU',
'BLEUW',
'BLFS',
'BLFY',
'BLI',
'BLIN',
'BLKB',
'BLMN',
'BLNG',
'BLNGU',
'BLNGW',
'BLNK',
'BLNKW',
'BLPH',
'BLRX',
'BLSA',
'BLTS',
'BLTSU',
'BLTSW',
'BLU',
'BLUE',
'BLZE',
'BMAQ',
'BMAQR',
'BMAQW',
'BMBL',
'BMEA',
'BMRA',
'BMRC',
'BMRN',
'BNFT',
'BNGO',
'BNGOW',
'BNIX',
'BNIXW',
'BNNR',
'BNNRU',
'BNNRW',
'BNOX',
'BNR',
'BNSO',
'BNTC',
'BNTX',
'BOCN',
'BOCNU',
'BOCNW',
'BOKF',
'BOLT',
'BON',
'BOOM',
'BOSC',
'BOTJ',
'BOXL',
'BPAC',
'BPACU',
'BPACW',
'BPMC',
'BPOP',
'BPOPM',
'BPRN',
'BPTH',
'BPTS',
'BPYPM',
'BPYPN',
'BPYPO',
'BPYPP',
'BRAC',
'BRACR',
'BRAG',
'BRCN',
'BREZ',
'BREZR',
'BREZW',
'BRFH',
'BRID',
'BRIV',
'BRIVU',
'BRIVW',
'BRKH',
'BRKHU',
'BRKHW',
'BRKL',
'BRKR',
'BRLI',
'BRLIR',
'BRLIU',
'BRLIW',
'BRLT',
'BROG',
'BRP',
'BRPM',
'BRPMW',
'BRQS',
'BRTX',
'BRY',
'BRZE',
'BSBK',
'BSET',
'BSFC',
'BSGA',
'BSGAR',
'BSGAU',
'BSGM',
'BSKY',
'BSKYU',
'BSKYW',
'BSQR',
'BSRR',
'BSVN',
'BSY',
'BTAI',
'BTB',
'BTBD',
'BTBDW',
'BTBT',
'BTCS',
'BTCY',
'BTOG',
'BTRS',
'BTTX',
'BTWN',
'BTWNU',
'BTWNW',
'BTX',
'BUSE',
'BVS',
'BVXV',
'BWAC',
'BWACU',
'BWACW',
'BWAQ',
'BWAQR',
'BWAQU',
'BWAQW',
'BWAY',
'BWB',
'BWBBP',
'BWC',
'BWCAU',
'BWCAW',
'BWEN',
'BWFG',
'BWMN',
'BWMX',
'BWV',
'BXRX',
'BYFC',
'BYND',
'BYNO',
'BYNOU',
'BYRN',
'BYSI',
'BYTS',
'BYTSU',
'BYTSW',
'BZ',
'BZFD',
'BZFDW',
'BZUN',
'CAAS',
'CABA',
'CAC',
'CACC',
'CADL',
'CAKE',
'CALA',
'CALB',
'CALM',
'CALT',
'CAMP',
'CAMT',
'CAN',
'CAPR',
'CAR',
'CARA',
'CARE',
'CARG',
'CARV',
'CASA',
'CASH',
'CASI',
'CASS',
'CASY',
'CATC',
'CATY',
'CBAN',
'CBAT',
'CBAY',
'CBFV',
'CBIO',
'CBNK',
'CBRG',
'CBRL',
'CBSH',
'CBTX',
'CCAIU',
'CCAIW',
'CCAP',
'CCB',
'CCBG',
'CCCC',
'CCD',
'CCEL',
'CCEP',
'CCLP',
'CCMP',
'CCNC',
'CCNE',
'CCNEP',
'CCOI',
'CCRN',
'CCSI',
'CCTS',
'CCTSU',
'CCTSW',
'CCXI',
'CD',
'CDAK',
'CDAQ',
'CDAQU',
'CDAQW',
'CDEV',
'CDK',
'CDLX',
'CDMO',
'CDNA',
'CDNS',
'CDRO',
'CDROW',
'CDTX',
'CDW',
'CDXC',
'CDXS',
'CDZI',
'CDZIP',
'CEAD',
'CEADW',
'CECE',
'CEG',
'CELC',
'CELH',
'CELU',
'CELUW',
'CELZ',
'CEMI',
'CENN',
'CENQ',
'CENQU',
'CENQW',
'CENT',
'CENTA',
'CENX',
'CERE',
'CERN',
'CERS',
'CERT',
'CETX',
'CETXP',
'CEVA',
'CFB',
'CFBK',
'CFFE',
'CFFEW',
'CFFI',
'CFFN',
'CFFS',
'CFFSU',
'CFFSW',
'CFIV',
'CFIVU',
'CFIVW',
'CFLT',
'CFMS',
'CFRX',
'CFSB',
'CFVI',
'CFVIU',
'CFVIW',
'CG',
'CGABL',
'CGBD',
'CGC',
'CGEM',
'CGEN',
'CGNT',
'CGNX',
'CGO',
'CGRN',
'CGTX',
'CHCI',
'CHCO',
'CHDN',
'CHEF',
'CHEK',
'CHEKZ',
'CHI',
'CHK',
'CHKEL',
'CHKEW',
'CHKEZ',
'CHKP',
'CHMG',
'CHNG',
'CHNGU',
'CHNR',
'CHRS',
'CHRW',
'CHSCL',
'CHSCM',
'CHSCN',
'CHSCO',
'CHSCP',
'CHTR',
'CHUY',
'CHW',
'CHWA',
'CHWAW',
'CHX',
'CHY',
'CIDM',
'CIFR',
'CIFRW',
'CIGI',
'CIH',
'CIIG',
'CIIGW',
'CINC',
'CINF',
'CING',
'CINGW',
'CISO',
'CITE',
'CITEU',
'CITEW',
'CIVB',
'CIZN',
'CJJD',
'CKPT',
'CLAQ',
'CLAQW',
'CLAR',
'CLAY',
'CLAYU',
'CLAYW',
'CLBK',
'CLBS',
'CLBT',
'CLBTW',
'CLDX',
'CLEU',
'CLFD',
'CLGN',
'CLIN',
'CLINR',
'CLINU',
'CLINW',
'CLIR',
'CLLS',
'CLMT',
'CLNE',
'CLNN',
'CLNNW',
'CLOE',
'CLOER',
'CLOEU',
'CLOV',
'CLPS',
'CLPT',
'CLRB',
'CLRM',
'CLRMU',
'CLRMW',
'CLRO',
'CLSD',
'CLSK',
'CLSN',
'CLST',
'CLVR',
'CLVRW',
'CLVS',
'CLWT',
'CLXT',
'CMAX',
'CMAXW',
'CMBM',
'CMCA',
'CMCAU',
'CMCAW',
'CMCO',
'CMCSA',
'CMCT',
'CME',
'CMLS',
'CMMB',
'CMPI',
'CMPO',
'CMPOW',
'CMPR',
'CMPS',
'CMPX',
'CMRX',
'CMTL',
'CNCE',
'CNDT',
'CNET',
'CNEY',
'CNFR',
'CNFRL',
'CNGL',
'CNGLU',
'CNGLW',
'CNNB',
'CNOB',
'CNOBP',
'CNSL',
'CNSP',
'CNTA',
'CNTB',
'CNTG',
'CNTQ',
'CNTQW',
'CNTX',
'CNTY',
'CNXC',
'CNXN',
'COCO',
'COCP',
'CODA',
'CODX',
'COFS',
'COGT',
'COHR',
'COHU',
'COIN',
'COKE',
'COLB',
'COLI',
'COLIU',
'COLIW',
'COLL',
'COLM',
'COMM',
'COMS',
'COMSP',
'COMSW',
'CONN',
'CONX',
'CONXU',
'CONXW',
'COOL',
'COOLU',
'COOLW',
'COOP',
'CORT',
'CORZ',
'CORZW',
'COSM',
'COST',
'COUP',
'COVA',
'COVAU',
'COVAW',
'COWN',
'COWNL',
'CPAA',
'CPAAU',
'CPAAW',
'CPAQ',
'CPAQW',
'CPAR',
'CPARU',
'CPARW',
'CPHC',
'CPIX',
'CPLP',
'CPOP',
'CPRT',
'CPRX',
'CPSH',
'CPSI',
'CPSS',
'CPTAG',
'CPTAL',
'CPTN',
'CPTNW',
'CPZ',
'CRAI',
'CRBP',
'CRBU',
'CRCT',
'CRDF',
'CRDL',
'CRDO',
'CREC',
'CRECU',
'CRECW',
'CREG',
'CRESW',
'CRESY',
'CREX',
'CRGE',
'CRIS',
'CRKN',
'CRMD',
'CRMT',
'CRNC',
'CRNT',
'CRNX',
'CRON',
'CROX',
'CRSP',
'CRSR',
'CRTD',
'CRTO',
'CRTX',
'CRUS',
'CRVL',
'CRVS',
'CRWD',
'CRWS',
'CRXT',
'CRXTW',
'CRZN',
'CRZNU',
'CRZNW',
'CSBR',
'CSCO',
'CSCW',
'CSGP',
'CSGS',
'CSII',
'CSIQ',
'CSLMW',
'CSPI',
'CSQ',
'CSSE',
'CSSEN',
'CSSEP',
'CSTE',
'CSTL',
'CSTR',
'CSWC',
'CSWI',
'CSX',
'CTAQ',
'CTAQU',
'CTAQW',
'CTAS',
'CTBI',
'CTG',
'CTHR',
'CTIB',
'CTIC',
'CTKB',
'CTLP',
'CTMX',
'CTRE',
'CTRM',
'CTRN',
'CTSH',
'CTSO',
'CTXR',
'CTXRW',
'CTXS',
'CUBA',
'CUE',
'CUEN',
'CULL',
'CURI',
'CURIW',
'CUTR',
'CVAC',
'CVBF',
'CVCO',
'CVCY',
'CVET',
'CVGI',
'CVGW',
'CVLG',
'CVLT',
'CVLY',
'CVRX',
'CVT',
'CVV',
'CWBC',
'CWBR',
'CWCO',
'CWST',
'CXDO',
'CYAD',
'CYAN',
'CYBE',
'CYBR',
'CYCC',
'CYCCP',
'CYCN',
'CYN',
'CYRN',
'CYRX',
'CYT',
'CYTH',
'CYTHW',
'CYTK',
'CYTO',
'CYXT',
'CZNC',
'CZR',
'CZWI',
'DADA',
'DAIO',
'DAKT',
'DALN',
'DAOO',
'DAOOU',
'DAOOW',
'DARE',
'DATS',
'DATSW',
'DAVE',
'DAVEW',
'DAWN',
'DBGI',
'DBGIW',
'DBTX',
'DBVT',
'DBX',
'DCBO',
'DCFC',
'DCFCW',
'DCGO',
'DCGOW',
'DCOM',
'DCOMP',
'DCPH',
'DCRD',
'DCRDU',
'DCRDW',
'DCT',
'DCTH',
'DDI',
'DDOG',
'DECAU',
'DENN',
'DERM',
'DFFN',
'DFH',
'DGHI',
'DGICA',
'DGICB',
'DGII',
'DGLY',
'DGNU',
'DH',
'DHAC',
'DHACU',
'DHACW',
'DHBC',
'DHBCU',
'DHBCW',
'DHC',
'DHCA',
'DHCAU',
'DHCAW',
'DHCNI',
'DHCNL',
'DHHC',
'DHHCU',
'DHHCW',
'DHIL',
'DIBS',
'DICE',
'DILA',
'DILAU',
'DILAW',
'DIOD',
'DISA',
'DISAU',
'DISAW',
'DISH',
'DJCO',
'DKDCA',
'DKDCU',
'DKDCW',
'DKNG',
'DLCA',
'DLCAU',
'DLCAW',
'DLHC',
'DLO',
'DLPN',
'DLTH',
'DLTR',
'DMAC',
'DMAQ',
'DMLP',
'DMRC',
'DMTK',
'DNAA',
'DNAB',
'DNAC',
'DNAD',
'DNAY',
'DNLI',
'DNUT',
'DOCU',
'DOGZ',
'DOMO',
'DOOO',
'DORM',
'DOX',
'DOYU',
'DPCS',
'DPCSU',
'DPCSW',
'DPRO',
'DRAY',
'DRAYW',
'DRCT',
'DRCTW',
'DRIO',
'DRMA',
'DRMAW',
'DRRX',
'DRTS',
'DRTSW',
'DRTT',
'DRUG',
'DRVN',
'DSAC',
'DSACW',
'DSEY',
'DSGN',
'DSGX',
'DSKE',
'DSP',
'DSWL',
'DTEA',
'DTIL',
'DTOC',
'DTOCU',
'DTOCW',
'DTRT',
'DTRTU',
'DTSS',
'DTST',
'DUET',
'DUETW',
'DUNE',
'DUNEU',
'DUNEW',
'DUO',
'DUOL',
'DUOT',
'DVAX',
'DWAC',
'DWACU',
'DWACW',
'DWSN',
'DXCM',
'DXLG',
'DXPE',
'DXR',
'DXYN',
'DYAI',
'DYN',
'DYNS',
'DYNT',
'DZSI',
'EA',
'EAC',
'EACPU',
'EACPW',
'EAR',
'EAST',
'EBAC',
'EBACU',
'EBACW',
'EBAY',
'EBC',
'EBET',
'EBIX',
'EBMT',
'EBON',
'EBTC',
'ECOL',
'ECOR',
'ECPG',
'EDAP',
'EDIT',
'EDNC',
'EDNCU',
'EDNCW',
'EDRY',
'EDSA',
'EDTK',
'EDTX',
'EDTXU',
'EDTXW',
'EDUC',
'EEFT',
'EEIQ',
'EFOI',
'EFSC',
'EFSCP',
'EFTR',
'EGAN',
'EGBN',
'EGLE',
'EGLX',
'EGRX',
'EH',
'EHTH',
'EIGR',
'EJFA',
'EJFAU',
'EJFAW',
'EJH',
'EKSO',
'ELBM',
'ELDN',
'ELEV',
'ELMS',
'ELMSW',
'ELOX',
'ELSE',
'ELTK',
'ELYM',
'ELYS',
'EM',
'EMBC',
'EMBK',
'EMBKW',
'EMCF',
'EMKR',
'EML',
'EMLD',
'EMLDU',
'ENCP',
'ENCPU',
'ENCPW',
'ENDP',
'ENER',
'ENERR',
'ENERU',
'ENERW',
'ENG',
'ENJY',
'ENJYW',
'ENLV',
'ENOB',
'ENPH',
'ENSC',
'ENSG',
'ENTA',
'ENTF',
'ENTFU',
'ENTFW',
'ENTG',
'ENTX',
'ENTXW',
'ENVB',
'ENVX',
'EOLS',
'EOSE',
'EOSEW',
'EPAY',
'EPHY',
'EPHYU',
'EPHYW',
'EPIX',
'EPSN',
'EPZM',
'EQ',
'EQBK',
'EQIX',
'EQOS',
'EQRX',
'EQRXW',
'ERAS',
'ERES',
'ERESU',
'ERESW',
'ERIC',
'ERIE',
'ERII',
'ERYP',
'ESAC',
'ESACU',
'ESACW',
'ESBK',
'ESCA',
'ESEA',
'ESGR',
'ESGRO',
'ESGRP',
'ESLT',
'ESOA',
'ESPR',
'ESQ',
'ESSA',
'ESSC',
'ESSCR',
'ESSCU',
'ESSCW',
'ESTA',
'ETAC',
'ETACU',
'ETACW',
'ETNB',
'ETON',
'ETSY',
'ETTX',
'EUCR',
'EUCRU',
'EUCRW',
'EVAX',
'EVBG',
'EVCM',
'EVER',
'EVFM',
'EVGN',
'EVGO',
'EVGOW',
'EVGR',
'EVGRU',
'EVGRW',
'EVK',
'EVLO',
'EVLV',
'EVLVW',
'EVO',
'EVOJ',
'EVOJU',
'EVOJW',
'EVOK',
'EVOP',
'EWBC',
'EWCZ',
'EWTX',
'EXAI',
'EXAS',
'EXC',
'EXEL',
'EXFY',
'EXLS',
'EXPD',
'EXPE',
'EXPI',
'EXPO',
'EXTR',
'EYE',
'EYEN',
'EYES',
'EYESW',
'EYPT',
'EZFL',
'EZGO',
'EZPW',
'FA',
'FAMI',
'FANG',
'FANH',
'FARM',
'FARO',
'FAST',
'FAT',
'FATBB',
'FATBP',
'FATE',
'FATP',
'FATPW',
'FB',
'FBIO',
'FBIOP',
'FBIZ',
'FBMS',
'FBNC',
'FBRX',
'FCAP',
'FCBC',
'FCCO',
'FCEL',
'FCFS',
'FCNCA',
'FCNCO',
'FCNCP',
'FCRD',
'FCUV',
'FDBC',
'FDMT',
'FDUS',
'FEAM',
'FEIM',
'FELE',
'FEMY',
'FENC',
'FEXD',
'FEXDR',
'FEXDU',
'FEXDW',
'FFBC',
'FFBW',
'FFHL',
'FFIC',
'FFIE',
'FFIEW',
'FFIN',
'FFIV',
'FFNW',
'FFWM',
'FGBI',
'FGBIP',
'FGEN',
'FGF',
'FGFPP',
'FGI',
'FGIWW',
'FGMC',
'FGMCU',
'FGMCW',
'FHB',
'FHLT',
'FHLTU',
'FHTX',
'FIAC',
'FIACU',
'FIACW',
'FIBK',
'FICV',
'FICVU',
'FICVW',
'FINM',
'FINMU',
'FINMW',
'FINW',
'FISI',
'FISV',
'FITB',
'FITBI',
'FITBO',
'FITBP',
'FIVE',
'FIVN',
'FIXX',
'FIZZ',
'FKWL',
'FLAC',
'FLACU',
'FLACW',
'FLEX',
'FLGC',
'FLGT',
'FLIC',
'FLL',
'FLMN',
'FLMNW',
'FLNC',
'FLNT',
'FLUX',
'FLWS',
'FLXS',
'FLYW',
'FMAO',
'FMBH',
'FMIV',
'FMIVW',
'FMNB',
'FMTX',
'FNCB',
'FNCH',
'FNGR',
'FNHC',
'FNKO',
'FNLC',
'FNVT',
'FNVTU',
'FNVTW',
'FNWB',
'FNWD',
'FOCS',
'FOLD',
'FONR',
'FORA',
'FORD',
'FORM',
'FORR',
'FORTY',
'FOSL',
'FOSLL',
'FOUN',
'FOUNU',
'FOUNW',
'FOX',
'FOXA',
'FOXF',
'FOXW',
'FPAY',
'FRAF',
'FRBA',
'FRBK',
'FRBN',
'FRBNU',
'FRBNW',
'FREE',
'FREEW',
'FREQ',
'FRG',
'FRGAP',
'FRGI',
'FRHC',
'FRLA',
'FRLAU',
'FRLN',
'FRME',
'FRMEP',
'FROG',
'FRON',
'FRONU',
'FRONW',
'FRPH',
'FRPT',
'FRSG',
'FRSGU',
'FRSGW',
'FRSH',
'FRST',
'FRSX',
'FRW',
'FRWAU',
'FRWAW',
'FSBC',
'FSBW',
'FSEA',
'FSFG',
'FSLR',
'FSRD',
'FSRDW',
'FSRX',
'FSRXU',
'FSRXW',
'FSSI',
'FSSIU',
'FSSIW',
'FSTR',
'FSTX',
'FSV',
'FTAA',
'FTAAU',
'FTAAW',
'FTAI',
'FTAIN',
'FTAIO',
'FTAIP',
'FTCI',
'FTCV',
'FTCVU',
'FTCVW',
'FTDR',
'FTEK',
'FTFT',
'FTHM',
'FTII',
'FTIIU',
'FTIIW',
'FTNT',
'FTPA',
'FTPAU',
'FTPAW',
'FTRP',
'FTVI',
'FTVIU',
'FTVIW',
'FULC',
'FULT',
'FULTP',
'FUNC',
'FUND',
'FUSB',
'FUSN',
'FUTU',
'FUV',
'FVAM',
'FVCB',
'FWAC',
'FWBI',
'FWONA',
'FWONK',
'FWP',
'FWRD',
'FWRG',
'FXCO',
'FXCOR',
'FXCOW',
'FXNC',
'FYBR',
'GABC',
'GACQ',
'GACQW',
'GAIA',
'GAIN',
'GAINN',
'GAINZ',
'GALT',
'GAMB',
'GAMC',
'GAMCU',
'GAMCW',
'GAME',
'GAN',
'GANX',
'GASS',
'GATE',
'GATEU',
'GATEW',
'GBDC',
'GBIO',
'GBNH',
'GBNY',
'GBOX',
'GBRG',
'GBRGR',
'GBRGW',
'GBS',
'GBT',
'GCBC',
'GCMG',
'GCMGW',
'GCTK',
'GDEN',
'GDNR',
'GDNRU',
'GDNRW',
'GDRX',
'GDS',
'GDST',
'GDSTR',
'GDSTU',
'GDSTW',
'GDYN',
'GECC',
'GECCM',
'GECCN',
'GECCO',
'GEEX',
'GEEXU',
'GEEXW',
'GEG',
'GENC',
'GENE',
'GENQ',
'GENQU',
'GENQW',
'GEOS',
'GERN',
'GET',
'GEVO',
'GFAI',
'GFAIW',
'GFGD',
'GFGDR',
'GFGDU',
'GFGDW',
'GFS',
'GGAA',
'GGAAW',
'GGAL',
'GGE',
'GGGV',
'GGMC',
'GGMCW',
'GGPI',
'GGPIU',
'GGPIW',
'GGR',
'GGROW',
'GH',
'GHAC',
'GHACU',
'GHACW',
'GHIX',
'GHIXU',
'GHIXW',
'GHRS',
'GHSI',
'GIAC',
'GIACW',
'GIFI',
'GIGM',
'GIII',
'GIIX',
'GIIXU',
'GIIXW',
'GILD',
'GILT',
'GIPR',
'GIPRW',
'GIW',
'GIWWU',
'GIWWW',
'GLAD',
'GLAQ',
'GLAQU',
'GLAQW',
'GLBE',
'GLBL',
'GLBLU',
'GLBLW',
'GLBS',
'GLBZ',
'GLDD',
'GLEE',
'GLEEU',
'GLEEW',
'GLG',
'GLHA',
'GLHAW',
'GLLI',
'GLLIR',
'GLLIW',
'GLMD',
'GLNG',
'GLPG',
'GLPI',
'GLRE',
'GLSI',
'GLSPT',
'GLSPU',
'GLSPW',
'GLTO',
'GLUE',
'GLYC',
'GMAB',
'GMBL',
'GMBLP',
'GMBLW',
'GMBLZ',
'GMDA',
'GMFI',
'GMFIU',
'GMGI',
'GMTX',
'GMVD',
'GMVDW',
'GNAC',
'GNACU',
'GNACW',
'GNCA',
'GNFT',
'GNLN',
'GNOG',
'GNPX',
'GNSS',
'GNTA',
'GNTX',
'GNTY',
'GNUS',
'GO',
'GOBI',
'GOCO',
'GOEV',
'GOEVW',
'GOGL',
'GOGO',
'GOOD',
'GOODN',
'GOODO',
'GOOG',
'GOOGL',
'GOSS',
'GOVX',
'GOVXW',
'GP',
'GPAC',
'GPACU',
'GPACW',
'GPCO',
'GPCOW',
'GPP',
'GPRE',
'GPRO',
'GRAB',
'GRABW',
'GRAY',
'GRCL',
'GRCY',
'GRCYU',
'GREE',
'GREEL',
'GRFS',
'GRIL',
'GRIN',
'GRNA',
'GRNAW',
'GRNQ',
'GROM',
'GROMW',
'GROW',
'GRPH',
'GRPN',
'GRTS',
'GRTX',
'GRVI',
'GRVY',
'GRWG',
'GSAQ',
'GSAQU',
'GSAQW',
'GSBC',
'GSEV',
'GSEVU',
'GSEVW',
'GSHD',
'GSIT',
'GSM',
'GSMG',
'GSMGW',
'GSRM',
'GSRMR',
'GSRMU',
'GSRMW',
'GT',
'GTAC',
'GTACU',
'GTACW',
'GTBP',
'GTEC',
'GTH',
'GTHX',
'GTIM',
'GTLB',
'GTPA',
'GTPAU',
'GTPAW',
'GTPB',
'GTPBU',
'GTPBW',
'GTX',
'GTXAP',
'GTYH',
'GURE',
'GVCI',
'GVCIU',
'GVP',
'GWGH',
'GWII',
'GWIIW',
'GWRS',
'GXII',
'GXIIW',
'GYRO',
'HA',
'HAAC',
'HAACU',
'HAACW',
'HAFC',
'HAIA',
'HAIAU',
'HAIAW',
'HAIN',
'HALL',
'HALO',
'HAPP',
'HARP',
'HAS',
'HAYN',
'HBAN',
'HBANM',
'HBANP',
'HBCP',
'HBIO',
'HBNC',
'HBP',
'HBT',
'HCAR',
'HCARU',
'HCARW',
'HCAT',
'HCCI',
'HCDI',
'HCDIP',
'HCDIW',
'HCDIZ',
'HCIC',
'HCICU',
'HCICW',
'HCII',
'HCIIW',
'HCKT',
'HCM',
'HCMA',
'HCMAU',
'HCMAW',
'HCNE',
'HCNEU',
'HCNEW',
'HCP',
'HCSG',
'HCTI',
'HCVI',
'HCVIU',
'HCVIW',
'HCWB',
'HDSN',
'HEAR',
'HEES',
'HELE',
'HEPA',
'HEPS',
'HERA',
'HERAU',
'HERAW',
'HEXO',
'HFBL',
'HFFG',
'HFWA',
'HGBL',
'HGEN',
'HHGC',
'HHGCW',
'HHS',
'HIBB',
'HIFS',
'HIHO',
'HIII',
'HIIIU',
'HIIIW',
'HILS',
'HIMX',
'HITI',
'HIVE',
'HLAH',
'HLAHU',
'HLAHW',
'HLBZ',
'HLBZW',
'HLG',
'HLIT',
'HLMN',
'HLNE',
'HLTH',
'HMCO',
'HMCOU',
'HMCOW',
'HMNF',
'HMPT',
'HMST',
'HMTV',
'HNNA',
'HNNAZ',
'HNRG',
'HNST',
'HOFT',
'HOFV',
'HOFVW',
'HOLI',
'HOLX',
'HON',
'HONE',
'HOOD',
'HOOK',
'HOPE',
'HORI',
'HORIW',
'HOTH',
'HOUR',
'HOVNP',
'HOWL',
'HPK',
'HPKEW',
'HPLT',
'HPLTU',
'HPLTW',
'HQI',
'HQY',
'HRMY',
'HROW',
'HROWL',
'HRTX',
'HRZN',
'HSAQ',
'HSDT',
'HSIC',
'HSII',
'HSKA',
'HSON',
'HST',
'HSTM',
'HSTO',
'HTBI',
'HTBK',
'HTCR',
'HTGM',
'HTHT',
'HTIA',
'HTIBP',
'HTLD',
'HTLF',
'HTLFP',
'HTOO',
'HTOOW',
'HTZ',
'HTZWW',
'HUBG',
'HUDI',
'HUGE',
'HUIZ',
'HUMA',
'HUMAW',
'HURC',
'HURN',
'HUSN',
'HUT',
'HVBC',
'HWBK',
'HWC',
'HWCPZ',
'HWEL',
'HWELW',
'HWKN',
'HYAC',
'HYACU',
'HYACW',
'HYFM',
'HYMC',
'HYMCL',
'HYMCW',
'HYMCZ',
'HYPR',
'HYRE',
'HYW',
'HYZN',
'HYZNW',
'HZNP',
'IAC',
'IART',
'IAS',
'IBCP',
'IBEX',
'IBKR',
'IBOC',
'IBRX',
'IBTX',
'ICAD',
'ICCC',
'ICCH',
'ICCM',
'ICFI',
'ICHR',
'ICLK',
'ICLR',
'ICMB',
'ICPT',
'ICUI',
'ICVX',
'IDAI',
'IDBA',
'IDCC',
'IDEX',
'IDN',
'IDRA',
'IDXX',
'IDYA',
'IEA',
'IEAWW',
'IEP',
'IESC',
'IFBD',
'IFRX',
'IGAC',
'IGACU',
'IGACW',
'IGIC',
'IGMS',
'IGNY',
'IGNYU',
'IGNYW',
'IGTA',
'IGTAR',
'IGTAU',
'IGTAW',
'IHRT',
'III',
'IIII',
'IIIIU',
'IIIIW',
'IIIV',
'IIN',
'IINN',
'IINNW',
'IIVI',
'IIVIP',
'IKNA',
'IKT',
'ILMN',
'ILPT',
'IMAB',
'IMAC',
'IMACW',
'IMAQ',
'IMAQR',
'IMAQU',
'IMAQW',
'IMBI',
'IMBIL',
'IMCC',
'IMCR',
'IMGN',
'IMGO',
'IMKTA',
'IMMP',
'IMMR',
'IMMX',
'IMNM',
'IMOS',
'IMPL',
'IMPP',
'IMPPP',
'IMRA',
'IMRN',
'IMRNW',
'IMRX',
'IMTE',
'IMTX',
'IMTXW',
'IMUX',
'IMV',
'IMVT',
'IMXI',
'INAB',
'INBK',
'INBKZ',
'INBX',
'INCR',
'INCY',
'INDB',
'INDI',
'INDIW',
'INDP',
'INDT',
'INFI',
'INFN',
'INGN',
'INKA',
'INKAU',
'INKAW',
'INKT',
'INM',
'INMB',
'INMD',
'INNV',
'INO',
'INOD',
'INPX',
'INSE',
'INSG',
'INSM',
'INTA',
'INTC',
'INTE',
'INTEU',
'INTEW',
'INTG',
'INTU',
'INTZ',
'INVA',
'INVE',
'INVO',
'INVZ',
'INVZW',
'INZY',
'IOAC',
'IOACU',
'IOACW',
'IOBT',
'IONM',
'IONS',
'IOSP',
'IOVA',
'IPA',
'IPAR',
'IPAX',
'IPAXW',
'IPDN',
'IPGP',
'IPHA',
'IPSC',
'IPVIU',
'IPVIW',
'IPW',
'IPWR',
'IQ',
'IQMD',
'IQMDU',
'IQMDW',
'IRBT',
'IRCP',
'IRDM',
'IREN',
'IRIX',
'IRMD',
'IROQ',
'IRTC',
'IRWD',
'ISAA',
'ISEE',
'ISIG',
'ISLE',
'ISLEW',
'ISO',
'ISPC',
'ISPO',
'ISPOW',
'ISRG',
'ISSC',
'ISTR',
'ISUN',
'ITAQ',
'ITAQU',
'ITAQW',
'ITCI',
'ITHX',
'ITHXU',
'ITHXW',
'ITI',
'ITIC',
'ITOS',
'ITQ',
'ITQRU',
'ITQRW',
'ITRI',
'ITRM',
'ITRN',
'IVA',
'IVAC',
'IVCB',
'IVCBU',
'IVCP',
'IVCPU',
'IVCPW',
'IVDA',
'IVDAW',
'IXAQ',
'IXAQU',
'IXAQW',
'IXHL',
'IZEA',
'JACK',
'JAGX',
'JAKK',
'JAMF',
'JAN',
'JANX',
'JAQC',
'JAQCU',
'JAQCW',
'JAZZ',
'JBHT',
'JBLU',
'JBSS',
'JCIC',
'JCICU',
'JCICW',
'JCSE',
'JCTCF',
'JD',
'JFIN',
'JFU',
'JG',
'JGGC',
'JGGCR',
'JGGCU',
'JJSF',
'JKHY',
'JMAC',
'JMACU',
'JMACW',
'JMSB',
'JNCE',
'JOAN',
'JOBS',
'JOFF',
'JOFFU',
'JOFFW',
'JOUT',
'JRSH',
'JRVR',
'JSM',
'JSPR',
'JUGG',
'JUGGU',
'JUPW',
'JUPWW',
'JVA',
'JWAC',
'JWACR',
'JWEL',
'JYAC',
'JYNT',
'JZXN',
'KACL',
'KACLR',
'KACLU',
'KAII',
'KAIIU',
'KAIIW',
'KAIR',
'KAIRU',
'KAIRW',
'KALA',
'KALU',
'KALV',
'KARO',
'KAVL',
'KBAL',
'KBNT',
'KBNTW',
'KC',
'KDNY',
'KDP',
'KE',
'KELYA',
'KELYB',
'KEQU',
'KERN',
'KERNW',
'KFFB',
'KFRC',
'KHC',
'KIDS',
'KIII',
'KIIIU',
'KIIIW',
'KINS',
'KINZ',
'KINZU',
'KINZW',
'KIRK',
'KLAC',
'KLAQ',
'KLAQU',
'KLAQW',
'KLIC',
'KLTR',
'KLXE',
'KMDA',
'KMPH',
'KNBE',
'KNDI',
'KNSA',
'KNTE',
'KNTK',
'KOD',
'KOPN',
'KOSS',
'KPLT',
'KPLTW',
'KPRX',
'KPTI',
'KRBP',
'KRKR',
'KRMD',
'KRNL',
'KRNLW',
'KRNT',
'KRNY',
'KRON',
'KROS',
'KRT',
'KRTX',
'KRUS',
'KRYS',
'KSCP',
'KSI',
'KSICU',
'KSICW',
'KSPN',
'KTCC',
'KTOS',
'KTRA',
'KTTA',
'KTTAW',
'KURA',
'KVHI',
'KVSA',
'KVSC',
'KXIN',
'KYCH',
'KYCHU',
'KYMR',
'KZIA',
'KZR',
'LAAA',
'LAAAW',
'LAB',
'LABP',
'LAKE',
'LAMR',
'LANC',
'LAND',
'LANDM',
'LANDO',
'LARK',
'LASR',
'LATG',
'LATGU',
'LATGW',
'LAUR',
'LAWS',
'LAX',
'LAXXR',
'LAXXW',
'LAZR',
'LAZY',
'LBAI',
'LBBB',
'LBBBR',
'LBBBU',
'LBBBW',
'LBC',
'LBPH',
'LBPS',
'LBPSW',
'LBRDA',
'LBRDK',
'LBRDP',
'LBTYA',
'LBTYB',
'LBTYK',
'LCA',
'LCAA',
'LCAAU',
'LCAAW',
'LCAHU',
'LCAHW',
'LCAP',
'LCAPU',
'LCAPW',
'LCFY',
'LCFYW',
'LCID',
'LCNB',
'LCUT',
'LDHA',
'LDHAU',
'LDHAW',
'LE',
'LECO',
'LEDS',
'LEE',
'LEGA',
'LEGAU',
'LEGAW',
'LEGH',
'LEGN',
'LESL',
'LEXX',
'LEXXW',
'LFAC',
'LFACU',
'LFACW',
'LFLY',
'LFLYW',
'LFMD',
'LFMDP',
'LFST',
'LFTR',
'LFTRU',
'LFTRW',
'LFUS',
'LFVN',
'LGAC',
'LGACU',
'LGACW',
'LGHL',
'LGHLW',
'LGIH',
'LGMK',
'LGND',
'LGO',
'LGST',
'LGSTU',
'LGSTW',
'LGTO',
'LGTOU',
'LGTOW',
'LGVC',
'LGVCU',
'LGVCW',
'LGVN',
'LHAA',
'LHCG',
'LHDX',
'LI',
'LIAN',
'LIBY',
'LIBYU',
'LIBYW',
'LIDR',
'LIDRW',
'LIFE',
'LILA',
'LILAK',
'LILM',
'LILMW',
'LINC',
'LIND',
'LINK',
'LION',
'LIONU',
'LIONW',
'LIQT',
'LITE',
'LITM',
'LITT',
'LITTU',
'LITTW',
'LIVBU',
'LIVBW',
'LIVE',
'LIVN',
'LIXT',
'LIXTW',
'LIZI',
'LJAQ',
'LJAQU',
'LJAQW',
'LJPC',
'LKCO',
'LKFN',
'LKQ',
'LLL',
'LLNW',
'LMACA',
'LMACU',
'LMACW',
'LMAO',
'LMAOU',
'LMAOW',
'LMAT',
'LMB',
'LMDX',
'LMDXW',
'LMFA',
'LMNL',
'LMNR',
'LMPX',
'LMST',
'LNDC',
'LNSR',
'LNT',
'LNTH',
'LOAN',
'LOB',
'LOCO',
'LOGC',
'LOGI',
'LOOP',
'LOPE',
'LOTZ',
'LOTZW',
'LOV',
'LOVE',
'LPCN',
'LPLA',
'LPRO',
'LPSN',
'LPTH',
'LPTX',
'LQDA',
'LQDT',
'LRCX',
'LRFC',
'LRMR',
'LSBK',
'LSCC',
'LSEA',
'LSEAW',
'LSPRU',
'LSPRW',
'LSTR',
'LSXMA',
'LSXMB',
'LSXMK',
'LTBR',
'LTCH',
'LTCHW',
'LTRN',
'LTRPA',
'LTRPB',
'LTRX',
'LTRY',
'LTRYW',
'LUCD',
'LULU',
'LUMO',
'LUNA',
'LUNG',
'LUXA',
'LUXAU',
'LUXAW',
'LVAC',
'LVACU',
'LVACW',
'LVLU',
'LVO',
'LVOX',
'LVOXW',
'LVRA',
'LVRAU',
'LVRAW',
'LVTX',
'LWAY',
'LWLG',
'LX',
'LXEH',
'LXRX',
'LYEL',
'LYFT',
'LYL',
'LYLT',
'LYRA',
'LYTS',
'LZ',
'MAAQ',
'MAAQR',
'MAAQU',
'MAAQW',
'MACA',
'MACAU',
'MACAW',
'MACK',
'MACU',
'MACUU',
'MACUW',
'MANH',
'MANT',
'MAPS',
'MAPSW',
'MAQC',
'MAQCW',
'MAR',
'MARA',
'MARK',
'MARPS',
'MASI',
'MASS',
'MAT',
'MATW',
'MAXN',
'MAYS',
'MBCN',
'MBII',
'MBIN',
'MBINN',
'MBINO',
'MBINP',
'MBIO',
'MBNKP',
'MBOT',
'MBRX',
'MBTC',
'MBTCR',
'MBTCU',
'MBUU',
'MBWM',
'MCAA',
'MCAAU',
'MCAAW',
'MCAE',
'MCAER',
'MCAEU',
'MCAF',
'MCAG',
'MCAGR',
'MCBC',
'MCBS',
'MCFT',
'MCHP',
'MCHX',
'MCLD',
'MCRB',
'MCRI',
'MDB',
'MDGL',
'MDGS',
'MDGSW',
'MDIA',
'MDJH',
'MDLZ',
'MDNA',
'MDRR',
'MDRRP',
'MDRX',
'MDVL',
'MDWD',
'MDWT',
'MDXG',
'MDXH',
'ME',
'MEAC',
'MEACU',
'MEACW',
'MEDP',
'MEDS',
'MEIP',
'MEKA',
'MELI',
'MEOA',
'MEOAU',
'MEOAW',
'MEOH',
'MERC',
'MESA',
'MESO',
'METC',
'METCL',
'METX',
'METXW',
'MF',
'MFH',
'MFIN',
'MGEE',
'MGI',
'MGIC',
'MGNI',
'MGNX',
'MGPI',
'MGRC',
'MGTA',
'MGTX',
'MGYR',
'MHLD',
'MHUA',
'MICT',
'MIDD',
'MIGI',
'MILE',
'MILEW',
'MIME',
'MIND',
'MINDP',
'MINM',
'MIRM',
'MIRO',
'MIST',
'MITA',
'MITAU',
'MITAW',
'MITC',
'MITK',
'MITO',
'MKD',
'MKSI',
'MKTW',
'MKTWW',
'MKTX',
'MLAB',
'MLAC',
'MLAI',
'MLAIU',
'MLAIW',
'MLCO',
'MLKN',
'MLTX',
'MLVF',
'MMAT',
'MMLP',
'MMMB',
'MMSI',
'MMYT',
'MNDO',
'MNDT',
'MNDY',
'MNKD',
'MNMD',
'MNOV',
'MNPR',
'MNRO',
'MNSB',
'MNSBP',
'MNST',
'MNTK',
'MNTS',
'MNTSW',
'MNTV',
'MNTX',
'MOBQ',
'MOBQW',
'MODD',
'MODV',
'MOFG',
'MOGO',
'MOHO',
'MOLN',
'MOMO',
'MON',
'MONCU',
'MONCW',
'MOR',
'MORF',
'MORN',
'MOTS',
'MOVE',
'MOXC',
'MPAA',
'MPAC',
'MPACR',
'MPACU',
'MPACW',
'MPB',
'MPRA',
'MPRAU',
'MPRAW',
'MPWR',
'MQ',
'MRAI',
'MRAM',
'MRBK',
'MRCC',
'MRCY',
'MREO',
'MRIN',
'MRKR',
'MRM',
'MRNA',
'MRNS',
'MRSN',
'MRTN',
'MRTX',
'MRUS',
'MRVI',
'MRVL',
'MSAC',
'MSACW',
'MSBI',
'MSDA',
'MSDAU',
'MSDAW',
'MSEX',
'MSFT',
'MSGM',
'MSSAU',
'MSTR',
'MSVB',
'MTAC',
'MTACU',
'MTACW',
'MTBC',
'MTBCO',
'MTBCP',
'MTC',
'MTCH',
'MTCR',
'MTEK',
'MTEKW',
'MTEM',
'MTEX',
'MTLS',
'MTP',
'MTRX',
'MTRY',
'MTRYW',
'MTSI',
'MTTR',
'MU',
'MUDS',
'MUDSW',
'MULN',
'MURF',
'MURFW',
'MVBF',
'MVIS',
'MVST',
'MVSTW',
'MXCT',
'MXL',
'MYFW',
'MYGN',
'MYMD',
'MYNA',
'MYNZ',
'MYPS',
'MYPSW',
'MYRG',
'MYSZ',
'NAAC',
'NAACU',
'NAACW',
'NAII',
'NAOV',
'NARI',
'NATH',
'NATI',
'NATR',
'NAUT',
'NAVI',
'NBEV',
'NBIX',
'NBN',
'NBRV',
'NBSE',
'NBST',
'NBSTU',
'NBSTW',
'NBTB',
'NBTX',
'NCAC',
'NCACU',
'NCACW',
'NCBS',
'NCMI',
'NCNA',
'NCNO',
'NCSM',
'NCTY',
'NDAC',
'NDACU',
'NDACW',
'NDAQ',
'NDLS',
'NDRA',
'NDRAW',
'NDSN',
'NECB',
'NEGG',
'NEO',
'NEOG',
'NEON',
'NEPH',
'NEPT',
'NERV',
'NESR',
'NESRW',
'NEWT',
'NEWTL',
'NEWTZ',
'NEXI',
'NEXT',
'NFBK',
'NFE',
'NFLX',
'NGM',
'NGMS',
'NH',
'NHIC',
'NHTC',
'NICE',
'NICK',
'NISN',
'NIU',
'NKLA',
'NKSH',
'NKTR',
'NKTX',
'NLIT',
'NLITW',
'NLOK',
'NLSP',
'NLSPW',
'NLTX',
'NMFC',
'NMIH',
'NMMC',
'NMMCU',
'NMMCW',
'NMRD',
'NMRK',
'NMTC',
'NMTR',
'NN',
'NNAVW',
'NNBR',
'NNDM',
'NNOX',
'NOAC',
'NOACU',
'NOACW',
'NODK',
'NOTV',
'NOVN',
'NOVT',
'NOVV',
'NOVVW',
'NPAB',
'NPABU',
'NPABW',
'NPCE',
'NRAC',
'NRACU',
'NRACW',
'NRBO',
'NRC',
'NRDS',
'NRIM',
'NRIX',
'NRSN',
'NRSNW',
'NRXP',
'NRXPW',
'NSEC',
'NSIT',
'NSPR',
'NSSC',
'NSTG',
'NSTS',
'NSYS',
'NTAP',
'NTCT',
'NTES',
'NTGR',
'NTIC',
'NTLA',
'NTNX',
'NTRA',
'NTRB',
'NTRBW',
'NTRS',
'NTRSO',
'NTUS',
'NTWK',
'NUBIU',
'NURO',
'NUTX',
'NUVA',
'NUVL',
'NUWE',
'NUZE',
'NVAC',
'NVACR',
'NVACW',
'NVAX',
'NVCN',
'NVCR',
'NVCT',
'NVDA',
'NVEC',
'NVEE',
'NVEI',
'NVFY',
'NVIV',
'NVMI',
'NVNO',
'NVNOW',
'NVOS',
'NVSA',
'NVSAU',
'NVSAW',
'NVTS',
'NVVE',
'NVVEW',
'NVX',
'NWBI',
'NWE',
'NWFL',
'NWL',
'NWLI',
'NWPX',
'NWS',
'NWSA',
'NXGL',
'NXGLW',
'NXGN',
'NXPI',
'NXPL',
'NXPLW',
'NXST',
'NXTC',
'NXTP',
'NYMT',
'NYMTL',
'NYMTM',
'NYMTN',
'NYMTZ',
'NYMX',
'NYXH',
'OAS',
'OB',
'OBCI',
'OBLG',
'OBNK',
'OBSV',
'OBT',
'OCAX',
'OCAXW',
'OCC',
'OCCI',
'OCCIN',
'OCCIO',
'OCDX',
'OCFC',
'OCFCP',
'OCG',
'OCGN',
'OCSL',
'OCUL',
'OCUP',
'OCX',
'ODFL',
'ODP',
'OEG',
'OEPW',
'OEPWU',
'OEPWW',
'OESX',
'OFED',
'OFIX',
'OFLX',
'OFS',
'OGI',
'OHAA',
'OHAAU',
'OHAAW',
'OHPA',
'OHPAU',
'OHPAW',
'OIIM',
'OKTA',
'OLB',
'OLED',
'OLIT',
'OLITW',
'OLK',
'OLLI',
'OLMA',
'OLPX',
'OM',
'OMAB',
'OMCL',
'OMEG',
'OMER',
'OMEX',
'OMGA',
'OMIC',
'OMQS',
'ON',
'ONB',
'ONBPO',
'ONBPP',
'ONCR',
'ONCS',
'ONCT',
'ONCY',
'ONDS',
'ONEM',
'ONEW',
'ONTX',
'ONVO',
'ONYX',
'ONYXU',
'ONYXW',
'OP',
'OPBK',
'OPCH',
'OPEN',
'OPGN',
'OPHC',
'OPI',
'OPINL',
'OPK',
'OPNT',
'OPOF',
'OPRA',
'OPRT',
'OPRX',
'OPT',
'OPTN',
'ORGN',
'ORGNW',
'ORGO',
'ORGS',
'ORIA',
'ORIAW',
'ORIC',
'ORLY',
'ORMP',
'ORRF',
'ORTX',
'OSBC',
'OSIS',
'OSPN',
'OSS',
'OST',
'OSTK',
'OSTR',
'OSTRU',
'OSTRW',
'OSUR',
'OSW',
'OTEC',
'OTECU',
'OTECW',
'OTEX',
'OTIC',
'OTLK',
'OTLY',
'OTMO',
'OTMOW',
'OTRA',
'OTRAU',
'OTRAW',
'OTRK',
'OTRKP',
'OTTR',
'OVBC',
'OVID',
'OVLY',
'OXAC',
'OXACU',
'OXACW',
'OXBR',
'OXBRW',
'OXLC',
'OXLCL',
'OXLCM',
'OXLCO',
'OXLCP',
'OXLCZ',
'OXSQ',
'OXSQL',
'OXSQZ',
'OXUS',
'OXUSU',
'OXUSW',
'OYST',
'OZK',
'OZKAP',
'PAA',
'PAAS',
'PACB',
'PACW',
'PACX',
'PACXU',
'PACXW',
'PAFO',
'PAFOR',
'PAFOU',
'PAGP',
'PAHC',
'PALI',
'PALT',
'PANA',
'PANL',
'PANW',
'PAQC',
'PAQCU',
'PAQCW',
'PARA',
'PARAA',
'PARAP',
'PASG',
'PATI',
'PATK',
'PAVM',
'PAVMZ',
'PAX',
'PAYA',
'PAYO',
'PAYOW',
'PAYS',
'PAYX',
'PBAX',
'PBAXU',
'PBAXW',
'PBBK',
'PBFS',
'PBHC',
'PBIP',
'PBLA',
'PBPB',
'PBTS',
'PBYI',
'PCAR',
'PCB',
'PCCT',
'PCCTU',
'PCCTW',
'PCH',
'PCOM',
'PCRX',
'PCSA',
'PCSB',
'PCT',
'PCTI',
'PCTTU',
'PCTTW',
'PCTY',
'PCVX',
'PCX',
'PCXCU',
'PCXCW',
'PCYG',
'PCYO',
'PDCE',
'PDCO',
'PDD',
'PDEX',
'PDFS',
'PDLB',
'PDSB',
'PEAR',
'PEARW',
'PEBK',
'PEBO',
'PECO',
'PEGA',
'PEGR',
'PEGRU',
'PEGRW',
'PEGY',
'PENN',
'PEP',
'PEPL',
'PEPLU',
'PEPLW',
'PERI',
'PESI',
'PETQ',
'PETS',
'PETV',
'PETVW',
'PETZ',
'PFBC',
'PFC',
'PFDR',
'PFDRW',
'PFG',
'PFHD',
'PFIE',
'PFIN',
'PFIS',
'PFMT',
'PFSW',
'PFTA',
'PFTAU',
'PFTAW',
'PFX',
'PFXNL',
'PFXNZ',
'PGC',
'PGEN',
'PGNY',
'PGRW',
'PGRWU',
'PGRWW',
'PHAR',
'PHAS',
'PHAT',
'PHCF',
'PHIC',
'PHICW',
'PHIO',
'PHUN',
'PHUNW',
'PHVS',
'PI',
'PIII',
'PIIIW',
'PIK',
'PINC',
'PIRS',
'PIXY',
'PKBK',
'PKOH',
'PLAB',
'PLAOU',
'PLAY',
'PLBC',
'PLBY',
'PLCE',
'PLL',
'PLMI',
'PLMIU',
'PLMIW',
'PLMR',
'PLPC',
'PLRX',
'PLSE',
'PLTK',
'PLUG',
'PLUS',
'PLXP',
'PLXS',
'PLYA',
'PMCB',
'PMD',
'PME',
'PMGM',
'PMGMU',
'PMGMW',
'PMTS',
'PMVP',
'PNBK',
'PNFP',
'PNFPP',
'PNRG',
'PNT',
'PNTG',
'POAI',
'PODD',
'POET',
'POLA',
'PONO',
'PONOW',
'POOL',
'POSH',
'POW',
'POWI',
'POWL',
'POWRU',
'POWRW',
'POWW',
'POWWP',
'PPBI',
'PPBT',
'PPC',
'PPHP',
'PPHPR',
'PPHPU',
'PPHPW',
'PPIH',
'PPSI',
'PPTA',
'PPYA',
'PPYAW',
'PRAA',
'PRAX',
'PRCH',
'PRCT',
'PRDO',
'PRDS',
'PRFT',
'PRFX',
'PRGS',
'PRIM',
'PRLD',
'PRLH',
'PRLHU',
'PRLHW',
'PROC',
'PROCW',
'PROF',
'PROG',
'PROV',
'PRPH',
'PRPL',
'PRPO',
'PRQR',
'PRSO',
'PRSR',
'PRSRU',
'PRSRW',
'PRTA',
'PRTC',
'PRTG',
'PRTH',
'PRTK',
'PRTS',
'PRVA',
'PRVB',
'PSAG',
'PSAGU',
'PSAGW',
'PSEC',
'PSHG',
'PSMT',
'PSNL',
'PSTI',
'PSTV',
'PSTX',
'PT',
'PTC',
'PTCT',
'PTE',
'PTEN',
'PTGX',
'PTIC',
'PTICU',
'PTICW',
'PTIX',
'PTIXW',
'PTLO',
'PTMN',
'PTNR',
'PTOC',
'PTOCW',
'PTON',
'PTPI',
'PTRA',
'PTRS',
'PTSI',
'PTVE',
'PUBM',
'PUCK',
'PUCKU',
'PUCKW',
'PULM',
'PUYI',
'PVBC',
'PWFL',
'PWOD',
'PWP',
'PWPPW',
'PWUP',
'PWUPU',
'PWUPW',
'PXLW',
'PXS',
'PXSAP',
'PXSAW',
'PYCR',
'PYPD',
'PYPL',
'PYR',
'PYXS',
'PZZA',
'QCOM',
'QCRH',
'QDEL',
'QFIN',
'QH',
'QIPT',
'QK',
'QLGN',
'QLI',
'QLYS',
'QMCO',
'QNRX',
'QNST',
'QQQX',
'QRHC',
'QRTEA',
'QRTEB',
'QRTEP',
'QRVO',
'QSAM',
'QSI',
'QSIAW',
'QTEK',
'QTEKW',
'QTNT',
'QTRX',
'QTT',
'QUBT',
'QUIK',
'QUMU',
'QURE',
'RACB',
'RACY',
'RACYW',
'RADA',
'RADI',
'RAIL',
'RAIN',
'RAM',
'RAMMU',
'RAMMW',
'RAND',
'RANI',
'RAPT',
'RARE',
'RAVE',
'RBB',
'RBBN',
'RBCAA',
'RBCN',
'RBKB',
'RCAC',
'RCACU',
'RCACW',
'RCAT',
'RCEL',
'RCHG',
'RCHGU',
'RCHGW',
'RCII',
'RCKT',
'RCKY',
'RCLF',
'RCLFU',
'RCLFW',
'RCM',
'RCMT',
'RCON',
'RCRT',
'RCRTW',
'RDBX',
'RDBXW',
'RDCM',
'RDFN',
'RDHL',
'RDI',
'RDIB',
'RDNT',
'RDUS',
'RDVT',
'RDWR',
'REAL',
'REAX',
'REDU',
'REE',
'REEAW',
'REED',
'REFI',
'REFR',
'REG',
'REGI',
'REGN',
'REKR',
'RELI',
'RELIW',
'RELL',
'RELY',
'RENO',
'RENT',
'REPL',
'RETA',
'RETO',
'REVB',
'REVBW',
'REVE',
'REVEU',
'REVEW',
'REVH',
'REVHU',
'REVHW',
'REYN',
'RFAC',
'RFACR',
'RFACU',
'RFACW',
'RFIL',
'RGC',
'RGCO',
'RGEN',
'RGF',
'RGLD',
'RGLS',
'RGNX',
'RGP',
'RGTI',
'RGTIW',
'RIBT',
'RICK',
'RICO',
'RICOU',
'RICOW',
'RIDE',
'RIGL',
'RILY',
'RILYG',
'RILYK',
'RILYL',
'RILYM',
'RILYN',
'RILYO',
'RILYP',
'RILYT',
'RILYZ',
'RIOT',
'RIVN',
'RKDA',
'RKLB',
'RLAY',
'RLMD',
'RLYB',
'RMBI',
'RMBL',
'RMBS',
'RMCF',
'RMGC',
'RMGCU',
'RMGCW',
'RMNI',
'RMR',
'RMTI',
'RNA',
'RNAZ',
'RNDB',
'RNER',
'RNERW',
'RNLX',
'RNST',
'RNW',
'RNWK',
'RNWWW',
'RNXT',
'ROAD',
'ROC',
'ROCAU',
'ROCC',
'ROCG',
'ROCGU',
'ROCGW',
'ROCK',
'ROCL',
'ROIC',
'ROIV',
'ROIVW',
'ROKU',
'ROLL',
'ROLLP',
'ROOT',
'ROSE',
'ROSEU',
'ROSEW',
'ROST',
'ROVR',
'RPAY',
'RPD',
'RPHM',
'RPID',
'RPRX',
'RPTX',
'RRBI',
'RRGB',
'RRR',
'RSLS',
'RSSS',
'RSVR',
'RSVRW',
'RTL',
'RTLPO',
'RTLPP',
'RTLR',
'RUBY',
'RUN',
'RUSHA',
'RUSHB',
'RUTH',
'RVAC',
'RVACU',
'RVACW',
'RVLP',
'RVMD',
'RVNC',
'RVPH',
'RVPHW',
'RVSB',
'RVSN',
'RVSNW',
'RWAY',
'RWLK',
'RWODU',
'RXDX',
'RXRA',
'RXRAU',
'RXRX',
'RXST',
'RXT',
'RYAAY',
'RYTM',
'RZLT',
'SABR',
'SABRP',
'SABS',
'SABSW',
'SAFM',
'SAFT',
'SAGA',
'SAGAR',
'SAGAU',
'SAGE',
'SAIA',
'SAL',
'SALM',
'SAMA',
'SAMAU',
'SAMAW',
'SAMG',
'SANA',
'SANB',
'SANBW',
'SANG',
'SANM',
'SANW',
'SASR',
'SATL',
'SATLW',
'SATS',
'SAVA',
'SBAC',
'SBCF',
'SBET',
'SBFG',
'SBFM',
'SBFMW',
'SBGI',
'SBLK',
'SBNY',
'SBNYP',
'SBRA',
'SBSI',
'SBT',
'SBTX',
'SBUX',
'SCAQ',
'SCAQU',
'SCAQW',
'SCHL',
'SCHN',
'SCKT',
'SCLE',
'SCLEU',
'SCLEW',
'SCMA',
'SCMAU',
'SCOA',
'SCOAU',
'SCOAW',
'SCOB',
'SCOBU',
'SCOBW',
'SCOR',
'SCPH',
'SCPL',
'SCPS',
'SCRM',
'SCRMU',
'SCRMW',
'SCSC',
'SCTL',
'SCVL',
'SCWX',
'SCYX',
'SDAC',
'SDACU',
'SDACW',
'SDC',
'SDGR',
'SDH',
'SDIG',
'SEAC',
'SEAT',
'SEATW',
'SECO',
'SEDG',
'SEED',
'SEEL',
'SEER',
'SEIC',
'SELB',
'SELF',
'SENEA',
'SENEB',
'SERA',
'SESN',
'SEV',
'SEVN',
'SFBC',
'SFET',
'SFIX',
'SFM',
'SFNC',
'SFST',
'SFT',
'SGA',
'SGBX',
'SGC',
'SGEN',
'SGH',
'SGHL',
'SGHLU',
'SGHT',
'SGII',
'SGIIU',
'SGIIW',
'SGLB',
'SGLY',
'SGMA',
'SGML',
'SGMO',
'SGMS',
'SGRP',
'SGRY',
'SGTX',
'SHAC',
'SHACU',
'SHACW',
'SHBI',
'SHC',
'SHCA',
'SHCAU',
'SHCAW',
'SHCR',
'SHCRW',
'SHEN',
'SHIP',
'SHIPZ',
'SHLS',
'SHOO',
'SHQA',
'SHQAU',
'SHQAW',
'SHUA',
'SHUAU',
'SHUAW',
'SHYF',
'SIBN',
'SIDU',
'SIEB',
'SIEN',
'SIER',
'SIERU',
'SIERW',
'SIFY',
'SIGA',
'SIGI',
'SIGIP',
'SILC',
'SILK',
'SIMO',
'SINT',
'SIOX',
'SIRI',
'SISI',
'SITM',
'SIVB',
'SIVBP',
'SJ',
'SKIN',
'SKYA',
'SKYAW',
'SKYT',
'SKYW',
'SKYX',
'SLAB',
'SLAM',
'SLAMU',
'SLAMW',
'SLCR',
'SLCRU',
'SLCRW',
'SLDB',
'SLDP',
'SLDPW',
'SLGC',
'SLGCW',
'SLGG',
'SLGL',
'SLGN',
'SLHG',
'SLHGP',
'SLM',
'SLMBP',
'SLN',
'SLNG',
'SLNH',
'SLNHP',
'SLNO',
'SLP',
'SLRC',
'SLRX',
'SLS',
'SLVR',
'SLVRU',
'SLVRW',
'SMAP',
'SMBC',
'SMBK',
'SMCI',
'SMED',
'SMFL',
'SMFR',
'SMFRW',
'SMID',
'SMIH',
'SMIHU',
'SMIHW',
'SMIT',
'SMLR',
'SMMF',
'SMMT',
'SMPL',
'SMSI',
'SMTC',
'SMTI',
'SNAX',
'SNAXW',
'SNBR',
'SNCE',
'SNCR',
'SNCRL',
'SNCY',
'SND',
'SNDL',
'SNDX',
'SNES',
'SNEX',
'SNFCA',
'SNGX',
'SNOA',
'SNPO',
'SNPS',
'SNPX',
'SNRH',
'SNRHU',
'SNRHW',
'SNSE',
'SNT',
'SNTG',
'SNY',
'SOFI',
'SOFO',
'SOHO',
'SOHOB',
'SOHON',
'SOHOO',
'SOHU',
'SOLO',
'SOLOW',
'SOND',
'SONDW',
'SONM',
'SONN',
'SONO',
'SOPA',
'SOPH',
'SOTK',
'SOVO',
'SP',
'SPCB',
'SPCM',
'SPCMW',
'SPFI',
'SPI',
'SPK',
'SPKB',
'SPKBU',
'SPKBW',
'SPLK',
'SPNE',
'SPNS',
'SPOK',
'SPPI',
'SPRB',
'SPRC',
'SPRO',
'SPSC',
'SPT',
'SPTK',
'SPTKW',
'SPTN',
'SPWH',
'SPWR',
'SQFT',
'SQFTP',
'SQFTW',
'SQL',
'SQLLW',
'SRAD',
'SRAX',
'SRCE',
'SRCL',
'SRDX',
'SREV',
'SRGA',
'SRNE',
'SRPT',
'SRRA',
'SRRK',
'SRSA',
'SRSAU',
'SRSAW',
'SRTS',
'SRZN',
'SSAA',
'SSAAU',
'SSAAW',
'SSB',
'SSBI',
'SSBK',
'SSIC',
'SSKN',
'SSNC',
'SSNT',
'SSP',
'SSRM',
'SSSS',
'SSSSL',
'SSTI',
'SSYS',
'STAA',
'STAB',
'STAF',
'STBA',
'STCN',
'STEP',
'STER',
'STGW',
'STIM',
'STKL',
'STKS',
'STLD',
'STNE',
'STOK',
'STRA',
'STRC',
'STRCW',
'STRL',
'STRM',
'STRN',
'STRNW',
'STRO',
'STRR',
'STRRP',
'STRS',
'STRT',
'STSA',
'STSS',
'STSSW',
'STTK',
'STX',
'SUMO',
'SUMR',
'SUNW',
'SUPN',
'SURF',
'SURG',
'SURGW',
'SV',
'SVC',
'SVFA',
'SVFAU',
'SVFAW',
'SVFB',
'SVFC',
'SVFD',
'SVNA',
'SVNAU',
'SVNAW',
'SVRA',
'SVSVU',
'SVSVW',
'SVVC',
'SWAG',
'SWAGU',
'SWAGW',
'SWAV',
'SWBI',
'SWET',
'SWETU',
'SWETW',
'SWIM',
'SWIR',
'SWKH',
'SWKS',
'SWSS',
'SWSSU',
'SWSSW',
'SWTX',
'SWVL',
'SWVLW',
'SXTC',
'SY',
'SYBT',
'SYBX',
'SYNA',
'SYNH',
'SYNL',
'SYPR',
'SYRS',
'SYTA',
'SYTAW',
'SZZLU',
'SZZLW',
'TA',
'TACT',
'TAIT',
'TALK',
'TALKW',
'TALS',
'TANH',
'TANNI',
'TANNL',
'TANNZ',
'TAOP',
'TARA',
'TARS',
'TASK',
'TAST',
'TATT',
'TAYD',
'TBBK',
'TBCP',
'TBCPU',
'TBCPW',
'TBK',
'TBKCP',
'TBLA',
'TBLAW',
'TBLD',
'TBLT',
'TBLTW',
'TBNK',
'TBPH',
'TBSA',
'TBSAU',
'TBSAW',
'TC',
'TCAC',
'TCACU',
'TCACW',
'TCBC',
'TCBI',
'TCBIO',
'TCBK',
'TCBP',
'TCBPW',
'TCBS',
'TCBX',
'TCDA',
'TCFC',
'TCMD',
'TCOM',
'TCON',
'TCPC',
'TCRR',
'TCRT',
'TCRX',
'TCVA',
'TCX',
'TDUP',
'TEAM',
'TECH',
'TECTP',
'TEDU',
'TEKK',
'TEKKU',
'TEKKW',
'TELA',
'TENB',
'TENX',
'TER',
'TERN',
'TESS',
'TETC',
'TETCU',
'TETCW',
'TETE',
'TETEU',
'TETEW',
'TFFP',
'TFSL',
'TGA',
'TGAA',
'TGAAU',
'TGAAW',
'TGAN',
'TGLS',
'TGTX',
'TGVC',
'TGVCU',
'TGVCW',
'TH',
'THAC',
'THACU',
'THACW',
'THCA',
'THCAW',
'THCP',
'THCPU',
'THCPW',
'THFF',
'THMO',
'THRM',
'THRN',
'THRX',
'THRY',
'THTX',
'THWWW',
'TIG',
'TIGO',
'TIGR',
'TIL',
'TILE',
'TIOA',
'TIOAU',
'TIOAW',
'TIPT',
'TIRX',
'TITN',
'TIVC',
'TKLF',
'TKNO',
'TLGY',
'TLGYW',
'TLIS',
'TLRY',
'TLS',
'TLSA',
'TMC',
'TMCI',
'TMCWW',
'TMDI',
'TMDX',
'TMKR',
'TMKRU',
'TMKRW',
'TMPM',
'TMPMW',
'TMUS',
'TNDM',
'TNGX',
'TNON',
'TNXP',
'TNYA',
'TOAC',
'TOACU',
'TOACW',
'TOI',
'TOIIW',
'TOMZ',
'TOPS',
'TOUR',
'TOWN',
'TPBAW',
'TPG',
'TPIC',
'TPST',
'TPTX',
'TRDA',
'TREE',
'TRHC',
'TRIB',
'TRIN',
'TRIP',
'TRKA',
'TRKAW',
'TRMB',
'TRMD',
'TRMK',
'TRMR',
'TRNS',
'TRON',
'TRONU',
'TRONW',
'TROO',
'TROW',
'TRS',
'TRST',
'TRUE',
'TRUP',
'TRVG',
'TRVI',
'TRVN',
'TSAT',
'TSBK',
'TSC',
'TSCAP',
'TSCBP',
'TSCO',
'TSEM',
'TSHA',
'TSIB',
'TSIBU',
'TSIBW',
'TSLA',
'TSP',
'TSRI',
'TSVT',
'TTCF',
'TTD',
'TTEC',
'TTEK',
'TTGT',
'TTMI',
'TTNP',
'TTOO',
'TTSH',
'TTWO',
'TUEM',
'TUGC',
'TUGCU',
'TUGCW',
'TURN',
'TUSK',
'TVTX',
'TVTY',
'TW',
'TWCB',
'TWCBU',
'TWCBW',
'TWIN',
'TWKS',
'TWLV',
'TWLVU',
'TWLVW',
'TWNK',
'TWOU',
'TWST',
'TXG',
'TXMD',
'TXN',
'TXRH',
'TYME',
'TYRA',
'TZOO',
'TZPS',
'TZPSW',
'UAL',
'UBCP',
'UBFO',
'UBOH',
'UBSI',
'UBX',
'UCBI',
'UCBIO',
'UCL',
'UCTT',
'UDMY',
'UEIC',
'UEPS',
'UFCS',
'UFPI',
'UFPT',
'UG',
'UGRO',
'UHAL',
'UIHC',
'UK',
'UKOMW',
'ULBI',
'ULCC',
'ULH',
'ULTA',
'UMBF',
'UMPQ',
'UNAM',
'UNB',
'UNCY',
'UNIT',
'UNTY',
'UONE',
'UONEK',
'UPC',
'UPLD',
'UPST',
'UPTD',
'UPTDU',
'UPTDW',
'UPWK',
'URBN',
'URGN',
'UROY',
'USAK',
'USAP',
'USAU',
'USCB',
'USCT',
'USCTU',
'USCTW',
'USEG',
'USIO',
'USLM',
'USWS',
'USWSW',
'UTAA',
'UTAAU',
'UTAAW',
'UTHR',
'UTMD',
'UTME',
'UTRS',
'UTSI',
'UVSP',
'UXIN',
'VABK',
'VACC',
'VALN',
'VALU',
'VAQC',
'VAXX',
'VBFC',
'VBIV',
'VBLT',
'VBNK',
'VBOC',
'VBOCU',
'VBOCW',
'VBTX',
'VC',
'VCEL',
'VCKA',
'VCKAU',
'VCKAW',
'VCNX',
'VCSA',
'VCTR',
'VCXA',
'VCXAU',
'VCXAW',
'VCYT',
'VECO',
'VECT',
'VEEE',
'VELO',
'VELOU',
'VELOW',
'VENA',
'VENAR',
'VENAW',
'VEON',
'VERA',
'VERB',
'VERBW',
'VERI',
'VERO',
'VERU',
'VERV',
'VERX',
'VERY',
'VEV',
'VFF',
'VG',
'VGFC',
'VHNAU',
'VHNAW',
'VIA',
'VIASP',
'VIAV',
'VICR',
'VIEW',
'VIEWW',
'VIGL',
'VII',
'VIIAW',
'VINC',
'VINO',
'VINP',
'VIOT',
'VIR',
'VIRC',
'VIRI',
'VIRT',
'VIRX',
'VISL',
'VITL',
'VIVE',
'VIVK',
'VIVO',
'VJET',
'VKTX',
'VLAT',
'VLATW',
'VLCN',
'VLDR',
'VLDRW',
'VLGEA',
'VLNS',
'VLON',
'VLY',
'VLYPO',
'VLYPP',
'VMAR',
'VMCA',
'VMCAU',
'VMCAW',
'VMD',
'VMEO',
'VMGA',
'VMGAW',
'VNDA',
'VNET',
'VNOM',
'VOD',
'VOR',
'VORB',
'VORBW',
'VOXX',
'VPCB',
'VPCBU',
'VPCBW',
'VQS',
'VRA',
'VRAR',
'VRAY',
'VRCA',
'VRDN',
'VREX',
'VRM',
'VRME',
'VRMEW',
'VRNA',
'VRNS',
'VRNT',
'VRPX',
'VRRM',
'VRSK',
'VRSN',
'VRTS',
'VRTX',
'VS',
'VSAC',
'VSACU',
'VSACW',
'VSAT',
'VSEC',
'VSSYW',
'VSTA',
'VSTM',
'VTAQ',
'VTAQR',
'VTAQU',
'VTAQW',
'VTGN',
'VTIQ',
'VTIQU',
'VTIQW',
'VTNR',
'VTRS',
'VTRU',
'VTSI',
'VTVT',
'VTYX',
'VUZI',
'VVOS',
'VVPR',
'VWE',
'VWEWW',
'VWTR',
'VXRT',
'VYGR',
'VYNE',
'VYNT',
'WABC',
'WAFD',
'WAFDP',
'WAFU',
'WALD',
'WALDU',
'WALDW',
'WASH',
'WATT',
'WAVD',
'WAVE',
'WAVS',
'WAVSU',
'WAVSW',
'WB',
'WBA',
'WBD',
'WDAY',
'WDC',
'WDFC',
'WEJO',
'WEJOW',
'WEN',
'WERN',
'WETF',
'WEYS',
'WFCF',
'WFRD',
'WHF',
'WHLM',
'WHLR',
'WHLRD',
'WHLRP',
'WILC',
'WIMI',
'WINA',
'WING',
'WINT',
'WINV',
'WINVR',
'WINVU',
'WINVW',
'WIRE',
'WISA',
'WISH',
'WIX',
'WKEY',
'WKHS',
'WKME',
'WKSP',
'WKSPW',
'WLDN',
'WLFC',
'WMG',
'WMPN',
'WNEB',
'WNW',
'WOOF',
'WORX',
'WPRT',
'WRAP',
'WRLD',
'WSBC',
'WSBCP',
'WSBF',
'WSC',
'WSFS',
'WSTG',
'WTBA',
'WTER',
'WTFC',
'WTFCM',
'WTFCP',
'WTMA',
'WTMAR',
'WTRH',
'WTW',
'WULF',
'WVE',
'WVFC',
'WVVI',
'WVVIP',
'WW',
'WWAC',
'WWACU',
'WWACW',
'WWD',
'WYNN',
'XAIR',
'XBIO',
'XBIT',
'XCUR',
'XEL',
'XELA',
'XELAP',
'XELB',
'XENE',
'XENT',
'XERS',
'XFIN',
'XFINU',
'XFINW',
'XFOR',
'XGN',
'XLO',
'XM',
'XMTR',
'XNCR',
'XNET',
'XOMA',
'XOMAO',
'XOMAP',
'XOS',
'XOSWW',
'XP',
'XPAX',
'XPAXU',
'XPAXW',
'XPDB',
'XPDBU',
'XPDBW',
'XPEL',
'XPER',
'XPON',
'XRAY',
'XRTX',
'XRX',
'XSPA',
'XTLB',
'XXII',
'YELL',
'YGMZ',
'YI',
'YJ',
'YMAB',
'YMTX',
'YORW',
'YOTAU',
'YQ',
'YTEN',
'YTRA',
'YVR',
'YY',
'Z',
'ZBRA',
'ZCMD',
'ZD',
'ZEAL',
'ZENV',
'ZEST',
'ZEUS',
'ZG',
'ZI',
'ZIMV',
'ZINGU',
'ZINGW',
'ZION',
'ZIONL',
'ZIONO',
'ZIONP',
'ZIVO',
'ZIVOW',
'ZKIN',
'ZLAB',
'ZM',
'ZNGA',
'ZNTE',
'ZNTEU',
'ZNTEW',
'ZNTL',
'ZS',
'ZSAN',
'ZT',
'ZTAQW',
'ZTEK',
'ZUMZ',
'ZVO',
'ZWRK',
'ZWRKW',
'ZY',
'ZYNE',
'ZYXI'




]
Oldest ={}
Old ={}
New ={}
Newest ={}

def findChar(Stock):
    l = 0
    value = ''
    charList = ['c', 'h', 'l', 'o']
    charNum = 0
    values = {}
    for i in range(len(Stock)):
        if Stock[i] == charList[charNum]:
            j = i+4
            l = priceLen(j, Stock)
        if l > 0:
            value += str(Stock[i+4])
            l -= 1
        if not value == '' and l == 0:
            values[charList[charNum]] = float(value)
            value = ''
            if(charNum < len(charList)-1):
                charNum += 1
    values['adjlow'] = 0
    values['adjopen'] = 0
    values['adjclose'] = 0
    values['adjhigh'] = 0
    return values


def priceLen(i, Stock):
    for j in range(i, len(Stock)):
        if Stock[j] == ',':
            return j-i









def get_all_values(nested_dictionary, old, currentTicker, Old, Oldest):
    c = -1
    h = -1
    l = -1
    n = -1
    num = 0
    for key, value in nested_dictionary.items():
        if type(value) is dict:
            currentTicker = key
            get_all_values(value, old, currentTicker, Old, Oldest)
        else:
            #print(old)
            if num == 3:
                n = value
                num += 1
            if num == 4 and not len(Oldest) == 0:
                dic = toTheBottom(old[currentTicker])
                nested_dictionary['adjopen'] = (dic['o']+dic['c'])/2
                nested_dictionary['adjlow'] = (l)
                nested_dictionary['adjhigh'] = (h)
                nested_dictionary['adjclose'] = (h+n+c+l)/4
                WhatToDo(Oldest, Old, dic, nested_dictionary, currentTicker)
                num += 1    
            if num == 2:
                l = value
                num += 1
            if num == 1:
                h = value
                num += 1 
            if num == 0:
                c = value
                num += 1
            

def toTheBottom(dic):
    for key, value in dic.items():
        if type(value) is dict:
            toTheBottom(value)
        else:
            return dic


def createStockList(all_stocks):
    stock_data = {}
    full_List = {}
    stock_data.clear()
    full_List.clear()
    api = tradeapi.REST(public_key, private_key, base_url)
    for stocks in all_stocks:
        t = api.get_bars(stocks, '1Hour', limit=5)
        if not t == []:
            stock_data[stocks] = t


    for stocks in stock_data:
        full_List[stocks] = findChar(str(stock_data[stocks]))
    return full_List

def WhatToDo(Os, Ol, N, Ns, currentTicker):
    if not len(Os) == 0:
        api = tradeapi.REST(public_key, private_key, base_url)
        Oldest = toTheBottom(Os[currentTicker])
        Old = toTheBottom(Ol[currentTicker])
        New = N
        Newest = Ns
        print(Oldest)
        print('\n')
        print(Old)
        print('\n')
        print(New)
        print('\n')
        print(Newest)
        print('\n')
        if Oldest['adjopen'] > Oldest['adjclose'] and Old['adjopen'] > Old['adjclose'] and New['adjopen'] > New['adjclose'] and Newest['adjopen'] < Newest['adjclose']:
            print('buy' + currentTicker)
            q = int(5000/Newest['adjclose'])

            
            api.submit_order(symbol=currentTicker, qty=q, side='buy', type='market', time_in_force='day')
        if Oldest['adjopen'] < Oldest['adjclose'] and Old['adjopen'] < Old['adjclose'] and New['adjopen'] < New['adjclose'] and Newest['adjopen'] > Newest['adjclose']:
            print('sell' + currentTicker)
            q = int(5000/Newest['adjclose'])
            
            api.submit_order(symbol=currentTicker, qty=q, side='sell', type='market', time_in_force='day')
        
while True:
    if((datetime.now().hour>=9)and (datetime.now().hour<=13.5)):
        Oldest.clear()
        Oldest.update(Old)
        Old.clear()
        Old.update(New)
        New.clear()
        New.update(Newest)
        #print(New)
        Newest.clear()
        Newest.update(createStockList(all_stocks))
        get_all_values(Newest, New, currentTicker, Old, Oldest)
        print('Loop')
        #WhatToDo(Oldest, Old, New, Newest, currentTicker)
        time.sleep(2000)
